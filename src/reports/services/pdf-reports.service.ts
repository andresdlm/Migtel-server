...
export class PdfReportsService {
  ...
  async getSalesBookReport(params: SalesBookReportDto, fileName?: string) {
    ...
    const pdf = await this.salesBookPdfService.generate(report, params, fileName);
    return pdf;
  }
  ...
  async getAccountReport(params: ReportDto, fileName?: string) {
    ...
    params.until.setDate(params.until.getDate() - 1);
    const pdf = await this.accountReportPdfService.generate(report, params, fileName);
    return pdf;
  }
  ...
  async getPaymentReport(params: PaymentReportDto, fileName?: string) {
    ...
    params.until.setDate(params.until.getDate() - 1);
    const pdf = await this.paymentPdfService.generate(typedReport, params, fileName);
    return pdf;
  }
  ...
  async getRetentionsReport(params: ReportDto, fileName?: string) {
    ...
    params.until.setDate(params.until.getDate() - 1);
    const pdf = await this.retentionsReportPdfService.generate(report, params, fileName);
    return pdf;
  }

  async getReferenceInvoiceReport(params: ReferenceDto, fileName?: string) {
    const report: ReferenceInvoice = await this.reportsService.getReferenceInvoiceReport(params);
    params.until.setDate(params.until.getDate() - 1);
    const pdf = await this.referenceInvoiceReportPdfService.generate(report, params, fileName);
    return pdf;
  }

  async getReferencePaymentReport(params: ReferenceDto, fileName?: string) {
    const report: ReferencePayment = await this.reportsService.getReferencePaymentReport(params);
    params.until.setDate(params.until.getDate() - 1);
    const pdf = await this.referencePaymentReportPdfService.generate(report, params, fileName);
    return pdf;
  }

  async getIgtfReport(params: ReportDto, fileName?: string) {
    const report: IgtfBook = await this.reportsService.getIgtfBookReport(params);
    params.until.setDate(params.until.getDate() - 1);
    const pdf = await this.igtfReportPdfService.generate(report, params, fileName);
    return pdf;
  }
  ...
  async getNotPaidInvoiceReport(fileName?: string) {
    ...
    const params = {};
    const pdf = await this.notPaidInvoicePdfService.generate(typedReport, params, fileName);
    return pdf;
  }

  async getConciliationInvoiceReport(params: SalesBookReportDto, fileName?: string) {
    const report: SalesBookReport = await this.reportsService.getConciliationReport(params);
    // params.until.setDate(params.until.getDate() - 1);
    const pdf = await this.conciliationInvoicePdfService.generate(report, params, fileName);
    return pdf;
  }
  ...
  async getPortalPaymentReport(params: PortalReportDto, fileName?: string) {
    ...
    // params.until.setDate(params.until.getDate() - 1);
    const pdf = await this.portalPaymentReportPdfService.generate(reportRes, params, fileName);
    return pdf;
  }
}
